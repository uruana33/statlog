...
func PackageData(buffCont []byte, kind string, biz string, falconData chan<- FalconStruct, formatDone chan<- struct{}) {
	...
	case "TypeNormal":
		...
		go func() {
			...
			falconFormat(statNormalBlock, biz, falconData, formatDone)
		}()
	...
	case "TypeSpecial":
		...
		go func() {
			falconFormat(statSpecialBlock, biz, falconData, formatDone)
		}()
